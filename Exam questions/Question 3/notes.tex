...
to travel from one point in space-time to another over a given time period, \textcolor{red}{or to travel with a given energy and momentum.}
...
\begin{align*}
    K(x'',t;x',t_0)& = \sum_{paths} e^{\frac{i}{\hbar} S[path]}\\
   & = \int \mathcal{D}[x(t)] e^{\frac{i}{\hbar} S[x(t)]}
\end{align*}
Where $S(x(t))=\int_{t_0}^{t_1} dt \mathcal{L}(x, \dot{x})$ is the action along a specific path, and \(\mathcal{D}[x(t)]\) denotes the integration over all possible paths \(x(t)\) connecting the initial and final points.
\begin{align*}
    K(x'',t;x',t_0)& = \sum_{paths} e^{\frac{i}{\hbar} S[path]}\\
...
By doing this we can express the path as a list of numbers instead of a continuous function ($x(t)$). These points are descrete and therefore it makes sense to sum over them. 
So now we can use the definition of the path integral expression $\mathcal{D}(x(t)) = \lim_{N\rightarrow\infty} \left(\frac{m}{2\pi i\hbar \Delta t}\right)^{N/2} \prod_{j=1}^{N-1} dx_j$.
The action can also be approximated as a sum over these discrete time steps:
\begin{align*}
    S[x(t)] &\approx \sum_{j=0}^{N-1} \epsilon \mathcal{L}\left(x_j, \frac{x_{j+1} - x_j}{\epsilon}\right) \\
    &= \sum_{j=0}^{N-1} \epsilon \frac{1}{2} m \left(\frac{x_{j+1} - x_j}{\epsilon}\right)^2 \\
    &= \sum_{j=0}^{N-1} \frac{m}{2\epsilon} (x_{j+1} - x_j)^2.
\end{align*}


\textcolor{red}{all of this below is very difficult to understand!!!!}
Now putting this all together we get the path integral expression for the propagator:
\begin{align*}
    K(x'',t;x',t_0) &= \lim_{N \to \infty} \left(\frac{m}{2\pi i \hbar \epsilon}\right)^{N/2} \int dx_1 \int dx_2 \cdots \int dx_{N-1} \\
    &\quad \times \exp\left\{\frac{i}{\hbar} \sum_{j=0}^{N-1} \frac{m}{2\epsilon} (x_{j+1} - x_j)^2\right\}.
\end{align*}
To evaluate this expression we make a change of variables to simplify the integrals. We define new variables \(y_j = x_{j+1} - x_j\) for \(j = 0, 1, \ldots, N-1\).
This transforms the integrals over \(x_j\) into integrals over \(y_j\), with the boundary conditions \(y_0 = x_1 - x'\) and \(y_{N-1} = x'' - x_{N-1}\).
The integrals over the \(y_j\) are Gaussian integrals, which can be evaluated using the standard result for Gaussian integrals.
After performing these integrals and taking the limit \(N \to \infty\), we obtain the final expression for the propagator of a free particle:
\begin{align*}
    K(x'',t;x',t_0) &= \sqrt{\frac{m}{2\pi i \hbar (t-t_0)}} e^{\frac{i}{\hbar}\frac{m(x''-x')^2}{2(t-t_0)}}.
\end{align*}
